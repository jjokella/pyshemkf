...
=pyshemkf= generates graphs and 2D-figures of variables and parameters
adapted to the typical output of SHEMAT-Suite.
...
*** =output/=
Directory for output of all tags (see [[*Tags][Tags]]). =output= has one
subdirectory for each tag. Each of these tag-subdirectories contains
subdirectories, whose names are file endings: =npy/=, =png/=, =pdf/=
and =eps/=. to save the corresponding files. In each of these
directories, output files of the corresponding file type are
save. Example:
#+BEGIN_SRC sh
...
contain only numpy arrays in =npy/=. =dists= contains RMSE
distributions, =specs= contains specifications of the simulated model
(for example the discretization).
*** =/scripts=
...
of to the function definitions of =pskf=. The following tags exist in
=pyshemkf=:
- =analysis=
- =errorplot=
- =forward=
...
- =uindex=
** analysis
2D-Images of variable means and realisation from EnKF-simulations in
SHEMAT-Suite.
** errorplot
Figures plotting RMSE mean vs EnKF-method.
** forward
2D-Images of variables in forward runs of SHEMAT-Suite.
** gaussianity
RMSE distrubutions from EnKF-simulations in SHEMAT-Suite.
** monitor
Visualizing monitoring point outpt from SHEMAT-Suite.
** numcomp
Matrix plots visualizing RMSE statistics.
** presvel
Visualizing prescribed velocity fields in SHEMAT-Suite.
** sensitivity
Sensitivity study utilities.
** singlecell
Visualizing single cell output from SHEMAT-Suite.
** uindex
Visualizing variable values at different units.
* Scripts
** endresread.ipy
