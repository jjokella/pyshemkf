...
directory]]).
* Tutorial
Getting started with =pyshemkf=. This tutorial walks you through the
usage of a typical script of =pyshemkf=.
...
At first, some Python modules are loaded
#+BEGIN_SRC python
  import numpy as np
...
After the switches, some output specifications are set. These
specifications include the model name, the date and letter of the
output directory and possibly specifications needed for the output at
hand. In the case of =forward=, this is the name of the variable to be
...
script. Now please run the IPython script
#+BEGIN_SRC sh
  ipython $HOME/pyshemkf/scripts/forward/runplot.ipy
#+END_SRC
This should produce no output, since all switches are set to zero. It
is rather a test if all Python modules are properly loaded. If you
receive a =ImportError=, you should do one of the following two
things:
1. =ImportError= for =numpy= or =matplotlib=: Check your general
   Python setup.
2. =ImportError= for =pskf=: Most probably, =PYTHONPATH= is not set,
   see [[*Prerequisites][Prerequisites]]
If there is simply no output, you can move forward to the next step.
** Reading SHEMAT-Suite output
If all Python modules are loaded correctly, we can start by setting
the =is_read= switch to =1=.
#+BEGIN_SRC python
  # Switches
  is_read = 1
  is_plot = 0
  ...
  is_backup = 0
#+END_SRC
If you re-run the script now, this will yield the following error:
#+BEGIN_SRC sh
...
specifications. Thus, the error tells us which output file the script
is looking for and how the SHEMAT-Suite output directories should be
structured.

To find the right file we need to:
1. Set up the correct SHEMAT-Suite output directories (see [[*SHEMAT-Suite output
    directory][SHEMAT-Suite output directory]])
2. Put the right specifications under =# Specs=.
If this still does not work, the function =fr.read= offers the
possibility to specify directory and name of a given vtk-output by
...
#+END_SRC
If this is ready, you can move on to plotting.
** Plotting
...
#+BEGIN_SRC python
  ...
  is_backup = 0
#+END_SRC
This should work with the same specifications that the output was read
with. While calling the plotting routine, the appearance of the plot
can be influenced through input parameters.
#+BEGIN_SRC python
  # Plot
  if is_plot:
