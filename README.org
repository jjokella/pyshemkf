...
#+END_SRC
** SHEMAT-Suite output directory structure
The Python-Scripts need a certain SHEMAT-Suite output directory
structure to be able to read output files. A single output directory
should look as follows:
#+BEGIN_SRC sh
  $HOME/shematOutputDir/<model_name>_output/<dat>/<dat>_<letter>
#+END_SRC
...
#+END_SRC
Inside one of the SHEMAT-Suite output directories, input files are
saved alongside output directories.

Input files
- general input file =MODELNAME= (=WAVEREAL=)
- true input file =MODELNAME_TRUE= (=WAVEREAL_TRUE=)
- EnKF input file =MODELNAME.enkf= (=WAVEREAL.enkf=)
- SGSim input fiels =sgsim_k_modelname_true.par=
  (=sgsim_k_wavereal_true=), =sgsim_k_modelname.par=
  (=sgsim_k_wavereal=)
Output directories
- =samples_output=
- =enkf_output=
