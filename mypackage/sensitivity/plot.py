...
###############################################################################
#        Plot sensitiviy curves for different parameters differences          #
###############################################################################

def dnplot(ax,model_name,dat,let,
               imons = [0,9],
               pic_format = 'pdf',              # png, eps, pdf
               title = "Default",
               position = [0.1,0.1,0.8,0.8],
    ...
    """
    A plotting function for temperature difference sensitivity scores.
    ...
    if not title=="Default":
        ...
        ax.set_title(
            'Sensitivity numbers at '+sa.obs_longlabels[imons[0]]+' minus '+sa.obs_longlabels[imons[1]],
            size = 20
            )
