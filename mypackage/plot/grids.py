...
def x(model_name):
    ...
    if model_name == 'headbctest' or model_name == 'alexdiff' or model_name == 'alexdiffbig':
        ...
        arr = np.array([0.005,0.015,0.025,0.035,0.045,0.055,0.065,0.075,0.085,0.095,
                            0.105,0.115,0.125,0.135,0.145,0.155,0.165,0.175,0.185,0.195,
                            0.205,0.215,0.225,0.235,            #0.01
                            0.2416, 0.24480000000000002, 0.248, 0.25120000000000003, 0.2544,
                            0.2576, 0.26080000000000003, 0.264, 0.2672, 0.27040000000000003,
                            0.2736, 0.2768, 0.28, 0.2832, 0.2864, 0.2896, 0.2928,
                            0.29600000000000004, 0.2992, 0.3024, 0.3056, 0.3088, 0.312,
                            0.31520000000000004, 0.3184, 0.3216, 0.32480000000000003, 0.328,
                            0.3312, 0.33440000000000003, 0.3376, 0.3408, 0.34400000000000003,
                            0.3472, 0.35040000000000004, 0.3536, 0.3568, 0.36, 0.3632, 0.3664,
                            0.36960000000000004, 0.3728, 0.376, 0.3792, 0.3824,
                            0.38560000000000005, 0.38880000000000003, 0.392, 0.3952, 0.3984,
                            0.4016, 0.40480000000000005, 0.40800000000000003, 0.4112, 0.4144,
                            0.4176, 0.4208, 0.42400000000000004, 0.4272, 0.4304, 0.4336, 0.4368,
                            0.44000000000000006, 0.44320000000000004, 0.4464, 0.4496, 0.4528,
                            0.456, 0.45920000000000005, 0.46240000000000003, 0.4656, 0.4688,
                            0.47200000000000003, 0.4752, 0.47840000000000005,
                            0.48160000000000003, 0.4848, 0.488, 0.4912, 0.49440000000000006,
                            0.49760000000000004, 0.5008, 0.504, 0.5072, 0.5104000000000001,
                            0.5136000000000001, 0.5168, 0.52, 0.5232, 0.5264, 0.5296000000000001,
                            0.5328, 0.536, 0.5392, 0.5424, 0.5456, 0.5488000000000001, 0.552,
                            0.5552, 0.5584, 0.5616,   #0.0032
                            0.5682, 0.5782, 0.5882000000000001,
                            0.5982000000000001, 0.6082000000000001, 0.6182000000000001,
                            0.6282000000000001, 0.6382000000000001, 0.6482, 0.6582, 0.6682,
                            0.6782, 0.6882, 0.6982, 0.7082, 0.7182000000000001,
                            0.7282000000000001, 0.7382000000000001, 0.7482, 0.7582, 0.7682,
                            0.7782, 0.7882, 0.7982])   # 0.01
    else:
...
def xticks(model_name):
    ...
    if model_name == 'headbctest' or model_name == 'alexdiff' or model_name == 'alexdiffbig':
        ...
        a = np.array([0+i*0.01 for i in range(25)])
        b = np.array([0.2432+i*0.0032 for i in range(101)])
        c = np.array([0.5732+i*0.01 for i in range(24)])
        arr = np.append(a,np.append(b,c))
    else:
        raise exceptions.RuntimeError('Model name not valid: '+model_name)
    return arr
...
def delx(model_name):
    ...
    """
    if model_name == 'headbctest' or model_name == 'alexdiff' or model_name == 'alexdiffbig':
        a = np.array([1.0 for i in range(4)])
        b = np.array([0.5 for i in range(4)])
        c = np.array([0.2 for i in range(5)])
        d = np.array([0.1 for i in range(10)])
        e = np.array([0.01 for i in range(51)])
        f = np.array([0.0032 for i in range(100)])
        g = np.array([0.01 for i in range(50)])   #a,b,c,d,e,f,g,d,c,b,a
        arr = np.append(a,np.append(b,np.append(c,np.append(d,np.append(e,np.append(f,np.append(g,np.append(d,np.append(c,np.append(b,a)))))))))) # [m]
    elif model_name == 'prescribedhead':
        arr = np.array([1.0 for i in range(31)])
    elif model_name == 'cubey':
        a = np.array([0.01 for i in range(24)])
        b = np.array([0.0032 for i in range(101)])
        c = np.array([0.01 for i in range(24)])
        arr = np.append(a,np.append(b,c))
    else:
        raise exceptions.RuntimeError('Model name not valid: '+model_name) 
    return arr
