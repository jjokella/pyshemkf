...
def make_arrows(ax, n_arrays, befaft, nrobs_int, arr_xvars, arr_yvars,
                color_arr):
...
def get_nrobs_int(fname, path):
    ...
    for i in range(20):
        ...
        if(line[0:11] == '# nrobs_int'):
            ...
            break
    if(n == 0):
        raise exceptions.RuntimeError('# nrobs_int not found in'
                                      + ' first 20 lines of: \n'
                                      + fname)
    f.close()
    return n
...
def get_cbar_high(var, m_kz_std_high, m_cbar_kz_high, m_cbar_kz_res_high,
                  m_cbar_cor_high, m_cbar_lz_high):
    """
...
def get_cbar_low(var, m_kz_std_low, m_cbar_kz_low, m_cbar_kz_res_low,
                 m_cbar_cor_low, m_cbar_lz_low):
    """
    Returns high and low values for certain variables.
    """
    if var == 'kz_mean' or var == 'kz':
...
def m_input_check(nfiles,
    ...
    for i in range(nfiles):
        if not (is_var_in_file(assim_variables_dir,
                               input_file_name_stems[i]
                               + str(m_first).zfill(4) + '.vtk',
                               varnames[i],
                               raise_io_error=0)
                or
                is_var_in_file(samples_out_dir,
                               ...
                               raise_io_error=0)):
            if not is_var_in_file(assim_variables_dir,
                                  input_file_name_stems[i] + 'param_'
                                  + str(m_first).zfill(4) + '.vtk',
                                  varnames[i],):
    ...
    if(npics > nrobs_int):
        raise exceptions.RuntimeError('Too high number of observations called.'
                                      + ' Check '
                                      + 'm_first, m_diff')
