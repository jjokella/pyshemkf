...
from mypackage.tools.run.pythonmodule as pm

# Paths
python_dir = pm.python_dir
input_file_dir = pm.python_exec_dir+'/'+'pp'


def get_zsp_kalman(ip, dp, xs, ns, Gss_prior, zs_prior, Hy, R, zy, Gyy_prior,
                   Gys_prior, Gssy):

    # -----------------------------------------
    # specific pilot point setup
    # -----------------------------------------
    jp = np.arange(0, ip+1, dp)  # position indices of pilot points
    # up to position index ip
    ...
    # N = Is-Ps
    # K = np.dot(np.dot(Gps_prior, Hy_prior.T), la.inv(Gyy_prior + R_prior))
    # Kalman gain
    # zp_post = zp_prior + np.dot( K ,  zy_prior-np.dot(Hy_prior,zs_prior) )
    # step1_prior = np.dot( la.inv(Gyy_prior+R_prior),
    # zy_prior-np.dot(Hy_prior,zs_prior))
    # step2_prior = np.dot(Hy_prior.T,step1_prior)
    # step3_prior = np.dot(Gps_prior,step2_prior)
    zp_post = zp_prior + np.dot(Gps_prior,
                                np.dot(Hy_prior.T,
                                       np.dot(la.inv(Gyy_prior+R_prior),
                                              zy_prior-np.dot(Hy_prior,
                                                              zs_prior))))
    #
    ...
    # Johannes Kalman
    zsp_post_j = zs_prior + np.dot(
        Ps,
        np.dot(Gss_prior,
               np.dot(Ps.T,
                      np.dot(Hy_prior.T,
                             np.dot(la.inv(Gyy_prior + R),
                                    zy_prior - np.dot(Hy_prior,
                                                      zs_prior))))))
    Gss_post_j = Gss_prior - np.dot(
        Ps,
        np.dot(Gss_prior,
               np.dot(Ps.T,
                      np.dot(Hy_prior.T,
    ...
    estvarp_j = np.diag(Gss_post_j).reshape(ns, 1)
    # estimation variance of s plus error term

    return (jp, zsp_post, estvar_j, estvarp_j, xp, zp_post, zsp_post_j,
            Gss_post_j)
    # global zp_post, zsp_post, Gpp_prior, Gsp_prior,
    # Gyy_prior, R_prior, Hy_prior,zy_prior
...
# Function that outputs several arrays according to how
# many pilot points (pps) are in use
def get_zsp(ip, dp, xs, ns, Gss, beta_pri, Hy, Gyy, R, zy, Xy, Xs, Gssy, Gsy,
            Gys):
    ...
    # specific pilot point setup
    # -----------------------------------------
    jp = range(0, ip+1, dp)  # position inds of pps up to position ind ip
    xp = xs[jp]           # positions of pps up to position ind ip
    nploc = xp.size          # number of first ip pps
    ...
    zp = beta_pri*Xp + np.dot(Gps,
                              np.dot(Hy.T,
                                     np.dot(la.inv(Gyy+R), zy-beta_pri*Xy)))
    zsp = beta_pri*Xs + np.dot(Gsp, np.dot(la.inv(Gpp), zp-beta_pri*Xp))
    #
    Gsspy = Gssy + np.dot(N,
                          np.dot(Gsy,
                                 np.dot(la.inv(Gyy + R), np.dot(Gys, N.T))))
    # ATTENTION!!! THIS one would be WRONG:
    # Gsspy2 = Gss - np.dot(Ps,
    #                       np.dot(Gsy,
    #                              np.dot(la.inv(Gyy + R), np.dot(Gys, Ps.T))))

    estvar = np.diag(Gssy).reshape(ns, 1)   # estimation variance of s
    estvarp = np.diag(Gsspy).reshape(ns, 1)  # est var of s plus error term

    #######################################################
    # Johannes updates
    zsp_johannes = beta_pri*Xs + np.dot(
        Ps,
        np.dot(Gss,
               np.dot(Ps.T,
                      np.dot(Hy.T,
    ...
    estvarp_johannes = np.diag(Gsspy_johannes).reshape(ns, 1)
    # estimation variance of s plus error term

    return (jp, zsp, Gsspy, estvar, estvarp, xp, zp, zsp_johannes,
            Gsspy_johannes, estvar_johannes, estvarp_johannes, zp_johannes)
...
####################################################################
    ...
    # estvar    = np.diag(Gssy).reshape(ns,1)   # estimation variance of s
    # estvarp   = np.diag(Gsspy).reshape(ns,1)  # esti var of s plus error term
    # aestvar   = np.mean(np.diag(Gssy)) # mean estmation variance of s
    # aestvarp  = np.mean(np.diag(Gsspy))
    # # mean estimation variance of s plus error term
    # adestvar  = np.mean(np.diag(np.dot(Hy,np.dot(Gssy,Hy.T))))
    # # mean estimation variance data
    # adestvarp = np.mean(np.diag(np.dot(Hy,np.dot(Gsspy,Hy.T))))
    # # mean estimation variance data plus error term
    # cestvar   = np.dot(c.T,np.dot(Gssy,c))
    # # estimation variance of c
    # cestvarp  = np.dot(c.T,np.dot(Gsspy,c))
    # # estimation variance of c
    # # destvar   = stats.gmean(la.eig(Gssy))
    # # destvarp  = stats.gmean(la.eig(Gsspy))
...
# prior_adestvar = np.mean(np.diag(np.dot(np.dot(Hy,Gss), Hy.T)))
# mean estimation variance of data
# prior_cestvar  = np.dot(np.dot(c.T,Gss), c) # estimation variance of c
# prior_cestvar = prior_cestvar[0,0]
# # prior_destvar  = stats.gmean(la.eig(Gss)[0][::-1])
# entropy of s
