...
from mypackage.tools.pp import calc
...
def make_plot(ip_last, dp, xs, ns, Gss, beta_pri, Hy, Gyy, R, zy, Xy, Xs, Gssy,
              Gsy, Gys, zs_rand, xy, privar, Qbb, Gss_prior, Gyy_prior,
              Gys_prior, zs_prior, estvar_prior, zs, zs_post, estvar_post,
              varss, ips, time_time, is_animate, is_kalman):
    # Calculate the matrices for all pilot points
    if is_kalman:
        jp, zsp, estvar, estvarp_j, xp, zp, zsp_post_j, Gss_post_j =\
            calc.get_zsp_kalman(ip_last, dp, xs, ns, Gss_prior, zs_prior,
                                Hy, R, zy, Gyy_prior, Gys_prior, Gssy)
        figtitle = 'Kalman'
    else:
        (jp,  zsp,  Gsspy,  estvar,  estvarp,  xp,  zp,
         zsp_johannes,  Gsspy_johannes,  estvar_johannes,
         estvarp_johannes,  zp_johannes) =\
            calc.get_zsp(ip_last, dp, xs, ns, Gss, beta_pri, Hy, Gyy, R, zy,
                         Xy, Xs, Gssy, Gsy, Gys)
        figtitle = 'Kriging'
    ...
    if not is_kalman:
        ...
        line7k,  = ax_k.plot(xs, zs_post+2.5*np.sqrt(estvar_post), ':',
                             color='b')
        line8k,  = ax_k.plot(xs, zs_post-2.5*np.sqrt(estvar_post), ':',
                             color='b')
    ...
    # line1_a,  = ax_a.plot(range(0, ip_last, dp),
    #                       aestvar_record[range(0, ip_last, dp)], 'b')
    # line2_a,  = ax_a.plot([0, nptot*dp],
    #                       [prior_aestvar, prior_aestvar], 'b:')
    # line3_a,  = ax_a.plot(range(0, ip_last, dp),
    #                       aestvarp_record[range(0, ip_last, dp)], 'r')

    # # plot the stupid criteria: ad
    # line1_ad,  = ax_ad.plot([0, nptot*dp],
    #                         [prior_adestvar, prior_adestvar], 'b:')
    # line2_ad,  = ax_ad.plot(range(0, ip_last, dp),
    #                         adestvarp_record[range(0, ip_last, dp)], 'r')

    # # plot the stupid criteria: c
    # line1_c,  = ax_c.plot([0, nptot*dp],
    #                       [prior_cestvar, prior_cestvar], 'b:')
    # line2_c,  = ax_c.plot(range(0, ip_last, dp),
    #                       cestvarp_record[range(0, ip_last, dp)], 'r')
    ...
    if not is_kalman:
        ...
        ax.set_xlabel('x')
        ax.set_label('s')
        ax.set_ylim(beta_pri
                    + 3*(np.sqrt(varss+Qbb+R[0, 0]))*np.array([-1, 1]))
    else:
        ax_k.set_title('Kalman: best estimate and 95% CI')
        ax_k.set_xlabel('x')
        ax_k.set_label('s')
        ax_k.set_ylim(beta_pri
                      + 3*(np.sqrt(varss+Qbb+R[0, 0]))*np.array([-1, 1]))
    ...
    def animate():
        ...
        for ip in ips:
            ...
            # get_zsp(ip)
            # get_zsp_kalman(ip)
            # (jp, zsp, estvar, estvarp, xp,
            #  zp, zsp_post_j, Gss_post_j) = calc.get_zsp_kalman(
            #      ip, dp, xs, ns, Gss_prior, zs_prior, Hy, R, zy,
            #      Gyy_prior, Gys_prior, Gssy)
            (jp,  zsp,  Gsspy,  estvar,  estvarp,  xp,  zp,
             zsp_johannes,  Gsspy_johannes,  estvar_johannes, estvarp_johannes,
             zp_johannes) = calc.get_zsp(ip, dp, xs, ns, Gss, beta_pri, Hy,
                                         Gyy, R, zy, Xy, Xs, Gssy, Gsy, Gys)
        ...
        # Time output
        print("Wall time: " + str(time.time()-time_time)
