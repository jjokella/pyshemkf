...
def head(x, z, A, B, L, z0):
    ...
    h: Numpy array
        3D array, head field.
    """
    head = A - B * (np.cosh(np.pi * z / L) / np.cosh(np.pi * z0 / L)) * np.cos(
        np.pi * x / L)
...
def temp(x, z, T1, qb, k, lam, rho, cp, mu, g, B, L, z0):
    ...
    cp : float
        volumetric (specific) heat capacity of
        the mixture of matrix and fluid
...
def conduction(z, T1, qb, lam, z0):
    """
    Compute conduction temperature field from Domenico (1978).
