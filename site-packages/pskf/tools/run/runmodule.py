...
def get_num_let(let):
    ...
    if len(let) == 3:
        return (26*26*(alphabet.index(let[0]))
                + 26*(alphabet.index(let[1]))
                + alphabet.index(let[2])
                + 26*26
                + 26)
    else:
...
#               CHANGE MATLAB
#############################################################
def simulate_dist(model_name, dist, is_true=False,
    ...
    log_file_name = (make_file_dir_names(model_name)[1]
                     ...
                     if not is_true else
                     make_file_dir_names(model_name)[1]
                     + '/' + make_file_dir_names(model_name)[7])

    # Save to text file
    np.savetxt(log_file_name,
               dist,
               comments='',
               header=('Target Histogram\n'
                       + '1\n'
                       + third_row+' '+str(np.mean(dist))
                       + ' +- '+str(np.std(dist))
...
def change_output_dir(model_name, old_dat, new_dat):
    ...
    """
    outdir = (os.environ['HOME']+"/shematOutputDir/"
              + model_name + "_output")

    # Rename root-date-directory
    if os.path.exists(outdir+"/"+old_dat):
        os.rename(outdir+"/"+old_dat,
                  outdir+"/"+new_dat)
    else:
...
def make_model_dir_tmp(model_name, letter, today):
    ...
    shutil.copytree(os.environ['HOME']+"/shematModelsDir/"
                    + model_name + '_model',
                    new_model_dir)
