...
Provides functions for manipulating strings, directories and
SHEMAT-Suite input files.
"""
import subprocess
import os
import shlex
...
def get_num_let(let):
    ...
    if len(let) == 3:
        ...
        raise RuntimeError('letter does not contain' +
                           ' 1,2 or 3 letters')
...
def read_records_input(file_name, hashtag_line):
    ...
    if records_not_exist_check == 1:
        raise RuntimeError('No records in Hashtag-Input: ' +
                           hashtag_line)
