...
def cb(cb_ax,
    ...
    if label_position == 'title':
        cb_ax.set_title(varlabels[varname], y=labelpad,
                        fontsize=labelsize)
    elif label_position == 'x':
        cb_ax.set_xlabel(varlabels[varname], labelpad=labelpad,
                         fontsize=labelsize)
    elif label_position == 'y':
        cb_ax.set_ylabel(varlabels[varname], labelpad=labelpad,
                         fontsize=labelsize)
    else:
        raise exceptions.RuntimeError('label_position wrong: '+label_position)
...
def my_hdf(fdir, fname, varname):
    ...
    if not varname in hdf_f.keys():
        raise exceptions.RuntimeError(var + ' not in ' + fname
                                    + '\n Dir: ' + fdir)

    # Dataset
    hdf_reader = hdf_f[varname]

    # Numpy array
    hdf_reader = np.array(hdf_reader[0])
