    ...
    if not only_scalar:
        for line in f:
            if line.find('VECTORS ' + var) > -1:
                ...
                return 1
    if raise_var_error:
        raise RuntimeError(var + ' not in ' + fname + '\n Dir: ' +
                           path)
    else:
        return 0
...
def cb(
    ...
    if label_position == 'title':
        ...
        raise RuntimeError('label_position wrong: ' +
                           label_position)
...
def movie(input_name_test='plot_m_b_01_100_av.png',
    ...
    if not os.path.isfile(input_name_test_abs):
        raise RuntimeError('FILE' + input_name_test_abs +
                           ' not found!!!')
...
def my_hdf(fname, varname):
    ...
    Returns
    ----------
    hdf_numpy : Numpy array
        Variable array from SHEMAT-Suite hdf5-output.
    """

    # Check existence of file.
    if not os.path.isfile(fname):
        raise RuntimeError(fname + ' not a file.')
    ...
    if varname not in hdf_f.keys():
        raise RuntimeError(varname + ' not in ' + fname)
